...
integration, including domain details, API endpoints, configuration keys,
default values, service definitions, and other shared parameters.
...
# Timeouts in seconds for API requests.
...
# --- Configuration & Options Flow Keys ---
# Keys used in the Home Assistant config entry data and options.
...
DEFAULT_SOURCE_TIMEZONE: Final = "Europe/Berlin"
...
# before the next set operation from the queue is processed.
SET_NODE_COOLDOWN_S: Final = 2.0

# Maximum size of the set_value queue. Prevents unbounded memory usage if API calls fail repeatedly.
...
# --- Service Definitions ---
# Service names registered with Home Assistant.
