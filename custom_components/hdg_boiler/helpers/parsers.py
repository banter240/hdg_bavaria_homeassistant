...
def _normalize_numeric_string(value_str: str, log_prefix: str) -> str:
    """Normalize a string containing a number by standardizing separators."""
    # Remove spaces which can be used as thousands separators
    value_str = value_str.replace(" ", "").replace("\u00a0", "")

    # If both dot and comma are present, assume the last one is the decimal separator
    if "." in value_str and "," in value_str:
        last_dot = value_str.rfind(".")
        last_comma = value_str.rfind(",")
        if last_comma > last_dot:  # European format (e.g., 1.234,56)
...
def _extract_numeric_string(raw_value: str, log_prefix: str) -> str | None:
    ...
    # 1. Strip common units from the end
    ...
    # 2. Normalize separators (e.g., 1.234,56 -> 1234.56)
    normalized_str = _normalize_numeric_string(value_no_units, log_prefix)

    # 3. Find the first numeric part in the normalized string
...
def _parse_number(
    ...
    if numeric_str is None:
        return None
    try:
        # Convert to float first to handle all cases, then to target type
...
def _parse_datetime(
    ...
    try:
        source_tz = ZoneInfo(timezone_str)
    ...
    # Try parsing ISO-like format with timezone offset
    try:
        # Remove colon from timezone offset for %z compatibility if present
        if ":" in value[-6:]:
            value = value[:-3] + value[-2:]
        dt_aware = datetime.strptime(value, "%Y-%m-%d %H:%M:%S%z")
...
def _convert_enum_text_to_key(
    ...
    if not translation_key or translation_key not in HDG_ENUM_TEXT_TO_KEY_MAPPINGS:
        return value

    enum_map = HDG_ENUM_TEXT_TO_KEY_MAPPINGS[translation_key]
    for text_val, key in enum_map.items():
        if text_val == value:
...
def parse_sensor_value(
    ...
    parser = _PARSER_MAP[parse_as_type]
    try:
        return parser(
            cleaned_value,
            log_prefix,
