...
__all__ = ["HdgPollingResponseProcessor"]
...
class HdgPollingResponseProcessor:
    ...
    def _is_recently_set(self, node_id: str) -> bool:
        ...
        last_set_time = self._coordinator._last_set_times.get(node_id, 0.0)
    ...
    def _should_ignore_polled_value(
        ...
        if current_value != parsed_polled_value:
            _LOGGER.warning(
                "Ignoring polled value '%s' for node '%s' in group '%s' due to recent set. "
                "Coordinator holds '%s'.",
    ...
    def _handle_duplicate_node_id(
        ...
        if existing_value != new_value:
            _PROCESSOR_LOGGER.error(
                "Duplicate base node ID '%s' (from API ID '%s') in group '%s' with "
                "conflicting values. Existing: '%s', New (skipped): '%s'.",
                node_id,
                api_id,
                group_key,
    ...
    def _process_single_item(
        ...
        if node_id in processed_ids and self._handle_duplicate_node_id(
            node_id, parsed_value, group_key, api_id
        ):
            return
